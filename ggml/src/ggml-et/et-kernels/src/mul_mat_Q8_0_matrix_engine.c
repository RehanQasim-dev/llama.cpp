...
// Hart 0: tensor engine compute (FMA, reduce, store). Supports fused MM+ADD:
...
//
// Vectorized: for each weight row m we gather 8 signed bytes at a time (fgb.ps
// sign-extends to 32-bit int), convert to FP32, scale by the block's fp16 d,
// and fscw.ps-scatter the 8 values down 8 panel lines (stride 64B) at column m.
...
int entry_point(struct ggml_et_mm_q8_params *params, void *env) {
    ...
    if (reuse_ok) {
        ...
        if (is_hart1) {
            ...
            // dispatch it reads a stale counter (>=1) and races ahead of the producer.
