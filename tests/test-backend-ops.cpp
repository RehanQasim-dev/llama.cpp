...
// perf mode: collect and print ET-SOC1 hardware performance counters per case
// (--et-perf-counters). Wired to the ET backend via optional reg proc-addresses,
// so it is a no-op for every other backend.
static bool g_et_perf_counters = false;

typedef void (*ggml_et_perf_begin_t)(void);
typedef void (*ggml_et_perf_end_t)(const char * label);
...
struct test_case {
    ...
    bool eval_perf(ggml_backend_t backend, const char * op_names_filter, printer * output_printer) {
        ...
        // optional ET hardware performance counters (no-op on other backends)
        ggml_et_perf_begin_t et_perf_begin = nullptr;
        ggml_et_perf_end_t   et_perf_end   = nullptr;
        if (g_et_perf_counters) {
            ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(ggml_backend_get_device(backend));
            if (reg) {
                et_perf_begin = (ggml_et_perf_begin_t) ggml_backend_reg_get_proc_address(reg, "ggml_backend_et_perf_counters_begin");
                et_perf_end   = (ggml_et_perf_end_t)   ggml_backend_reg_get_proc_address(reg, "ggml_backend_et_perf_counters_end");
            }
        }
        if (et_perf_begin) {
            et_perf_begin();
        ...
        // print the averaged hardware counters right after this case's perf line
        if (et_perf_end) {
            std::string label = current_op_name + "(" + vars() + ")";
            et_perf_end(label.c_str());
        }
...
static void usage(char ** argv) {
    ...
    printf("    --et-perf-counters (perf mode) print averaged ET-SOC1 hardware performance counters per case\n");
}
...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--et-perf-counters") == 0) {
            g_et_perf_counters = true;
        } else {
            usage(argv);
            return 1;
        }
