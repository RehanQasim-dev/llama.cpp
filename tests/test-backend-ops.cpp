...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // DeepSeek-V2-Lite MLA: NORMAL-mode RoPE on a non-contiguous q_pe/k_pe view.
    // Uses YaRN with a small freq_scale (factor 40 → fs ≈ 0.025) and ext_factor=1.0,
    // which is not covered by the matrix loops above (fs ∈ {1.0, 1.4245}, ef ∈ {0, 0.7465}).
    // Also requires n_ctx_orig != 0 (yarn correction-dim formula depends on it).
    // Shape mimics q_pe = {64, n_head, n_tokens, 1} and k_pe = {64, 1, n_tokens, 1}.
    // v=1 forces a strided dim-1 view (analogous to ggml_view_3d into the wider q tensor).
