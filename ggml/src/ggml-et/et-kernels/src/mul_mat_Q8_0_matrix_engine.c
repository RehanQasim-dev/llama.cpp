...
//
// Vectorized: for each weight row m we gather 8 packed bytes at a time, convert
// to FP32, scale by the block's fp16 d, and
...
dequant_q8_0_panel(float *panel, const char *src0_batch,
    ...
    for (int j = 0; j < TILE_M; ++j) {
        ...
        __asm__ volatile(
            ...
            "fgb.ps      f4, f2(%[qs0]) \n\t"   // gather qs[0..7] and sign extend to 32-bit int
...
// Preload one C row of the residual bias into the matrix-C accumulator.
//
// For TensorFMA32 (FP32) the C matrix lives in the *vector register file*
// (f0..f31), NOT in the L1 scratchpad or the dedicated TenC file (that is only
// used by the integer TensorIMA8A32). So C cannot be seeded with TensorLoad;
// it must be written with ordinary vector loads (flw.ps) into the f registers.
//
// Layout per the ISA: A-row n -> C-row n, and each 16-wide C row occupies two
// 8-lane registers: f(2*R) holds cols m=0..7, f(2*R+1) holds cols m=8..15.
// Running every FMA pass with first_pass=0 makes the FMA add A*B onto whatever
// is already in C, so seeding C with the bias yields C = bias + sum(A*B) for
// free — no separate add pass or extra tensor instruction.
...
int entry_point(struct ggml_et_mm_q8_params *params, void *env) {
    ...
    // Fused residual bias (MUL_MAT+ADD). NULL when unfused. Same shape/strides as
    // dst (enforced by the graph fuse check), so it is indexed exactly like dst.
    ...
    // ================================================================
    // Hart 1: Q8_0 weight dequant producer
    // ================================================================
    if (is_hart1) {
    ...
    for (int64_t tile = (int64_t) shire_id + local_tile_idx * NUM_COMPUTE_SHIRES;
        ...
        // Unfused (bias == NULL): the first FMA uses first_pass=1, which OVERWRITES
        // C := A*B, so C's prior contents don't matter. Fused: we seed C with the
        // bias here and run *every* FMA with first_pass=0, so C = bias + sum(A*B).
        //
        // Correctness note: the fused path is the only code that relies on f0..f31
        // surviving from here until the first FMA. That holds because the only
        // things between this seed and the FMA are scp_wait() and tensor_load()
        // (A/B), and neither touches FP registers (tensor_load clobbers only x31).
        // The unfused path never needed this guarantee (first_pass=1 overwrites C).
        int first = 1;
        ...
        if (bias_batch) {
            ...
            // flw.ps is masked by m0: it only loads the lanes m0 enables. The mask
            // can be left in any state by prior tensor ops, so force all 8 lanes on
            // (and restore it after) or bias rows load only partially -> output is
            // coherent-looking but subtly wrong.
            ...
            // n_cur==4 runs the FMA with AROWS==4 (5 rows) as an errata pad; its
            // 5th C row (f8,f9) is never stored but must not start from garbage
            // under first_pass=0, so zero it.
            if (n_cur == 4) {
                static const float __attribute__((aligned(64))) zero_line[8] = {0};
                __asm__ volatile(
                    "flw.ps f8, 0(%[z]) \n\t"
                    "flw.ps f9, 0(%[z]) \n\t"
                    : : [z] "r"(zero_line) : "f8", "f9", "memory");
            }
            // Restore the caller's m0 mask.
